...
def test_session_boundaries() -> None:
    ...
    _check("Asia 02:00", session_name_for_hour(2) == "Asia")
    _check("Pre-London 07:00", session_name_for_hour(7) == "Pre-London")
    _check("London 09:00", session_name_for_hour(9) == "London")
    _check("Pre-NY 12:00", session_name_for_hour(12) == "Pre-NY")
    _check("New York 14:00", session_name_for_hour(14) == "New York")
    _check("Pre-Asia 22:00", session_name_for_hour(22) == "Pre-Asia")
    _check("is_pre_session Pre-London", is_pre_session("Pre-London") == True)
    _check("is_pre_session London", is_pre_session("London") == False)
...
def test_session_ingestion_and_archive() -> None:
    ...
    _check("Asia session name", ctx.get("session") == "Asia")
    _check("Asia has POC", ctx.get("session_poc") is not None)
    _check("Asia has OHLC open", ctx.get("session_ohlc", {}).get("open") == 50000.0)

    sp.ingest(london_ts, 50200.0, 2.0, "buy")
    sp.ingest(london_ts + 1000, 50300.0, 1.5, "sell")

    ctx = sp.current_context()
    _check("London session name", ctx.get("session") == "London")

    archived = sp.get_archived_profiles()
    _check("Asia archived", len(archived) >= 1)
    if archived:
        _check("Archived label Asia", archived[0].label == "Asia")
        _check("Archived has POC", archived[0].poc is not None)
        _check("Archived has VA", archived[0].value_area_high is not None)
...
def test_session_initial_balance() -> None:
    ...
    ctx = sp.current_context()
    _check("IB high", ctx.get("initial_balance_high") == 50200.0)
    _check("IB low", ctx.get("initial_balance_low") == 50000.0)

    va_h, va_l = _compute_value_area({50000: 10, 50025: 20, 50050: 5, 50075: 3})
    _check("VA computed", va_h is not None and va_l is not None)
    if va_h is not None and va_l is not None:
        _check("VA high >= low", va_h >= va_l)
...
def test_poc_drift() -> None:
    ...
    ctx = sp.current_context()
    _check("POC drift buckets", ctx.get("poc_drift_buckets") is not None)
...
def test_weekly_boundaries() -> None:
    ...
    _check("Week starts Sunday", ws_dt.weekday() == 6)
    _check("Week starts 22:00", ws_dt.hour == 22)
    _check("Week label format", ws_label.startswith("Week-"))

    ts2 = int(datetime(2026, 6, 3, 14, 0, tzinfo=timezone.utc).timestamp() * 1000)
    ws2_ms, ws2_label = _week_start_ms(ts2)
    _check("Same week", ws_label == ws2_label)

    ts3 = int(datetime(2026, 6, 7, 22, 0, tzinfo=timezone.utc).timestamp() * 1000)
    ws3_ms, ws3_label = _week_start_ms(ts3)
    _check("Next week different", ws_label != ws3_label)
...
def test_weekly_ingest() -> None:
    ...
    _check("Week label present", ctx.get("week", "").startswith("Week-"))
    _check("Week has POC", ctx.get("week_poc") is not None)
    _check("Week has OHLC", ctx.get("week_ohlc", {}).get("open") is not None)
...
def test_profile_shape() -> None:
    ...
    _check("b-Shape", classify_shape(vap_b)["shape"] == "b")

    vap_p = {50000: 1, 50025: 2, 50050: 5, 50075: 10, 50100: 20}
    _check("P-Shape", classify_shape(vap_p)["shape"] == "P")

    vap_d = {50000: 5, 50025: 15, 50050: 20, 50075: 15, 50100: 5}
    _check("D-Shape", classify_shape(vap_d)["shape"] == "D")

    vap_b2 = {50000: 20, 50025: 5, 50050: 3, 50075: 5, 50100: 18}
    _check("B-Shape", classify_shape(vap_b2)["shape"] == "B")

    _check("Empty VAP unknown", classify_shape({})["shape"] == "unknown")
    _check("None VAP unknown", classify_shape(None)["shape"] == "unknown")
...
def test_absorption() -> None:
    ...
    _check("Small vol: no absorption", e is None)
    ...
    for i in range(30):
        ...
        if event:
            break
    _check("Absorption event", event is not None)
    if event:
        _check("Bullish (sell absorbed)", event["absorption_type"] == "bullish")
...
def test_divergence() -> None:
    ...
    _check("Bearish divergence detected", div is not None)
    if div is not None:
        _check("Type contains bearish", "bearish" in div["divergence_type"])
...
def test_composite_profile() -> None:
    ...
    _check("Composite has POC", ctx.get("composite_poc") is not None)
    _check("Composite count", ctx.get("profile_count") == 2)
    _check("Balance flag set", ctx.get("balance_flag", "") in ("balance", "imbalance_up", "imbalance_down"))

    composite_vap = average_vap([snap1, snap2])
    hvn = find_hvn_zones(composite_vap, threshold_pct=0.05)
    _check("HVN zones found", len(hvn) > 0)

    lvn = find_lvn_zones(composite_vap, max_vol_pct=0.05)
    _check("LVN zones found", len(lvn) > 0)
    if lvn:
        _check("LVN zone price_low < price_high", lvn[0]["price_low"] < lvn[0]["price_high"])

    overlap = check_value_area_overlap(snap1, snap2)
    _check("VA non-overlap detected", overlap["overlap"] == False)
...
def test_pre_session_anomaly() -> None:
    ...
    _check("Low volume: no anomaly", anomaly is None)
    ...
    _check("High volume: anomaly detected", anomaly2 is not None)
    if anomaly2 is not None:
        _check("Anomaly factor > threshold", anomaly2["anomaly_factor"] > 1.5)

    sp3 = SessionProfile()
    bl = sp3.build_pre_session_baseline_from_parquet(Path("data/historical"))
    _check("Build baseline returns 3 phases", len(bl) == 3)
    for phase in ["Pre-Asia", "Pre-London", "Pre-NY"]:
        _check(f"Baseline has {phase}", phase in bl)
        _check(f"  {phase} > 0", bl[phase] > 0)
